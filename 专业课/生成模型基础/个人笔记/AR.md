...
Accuracy means the approximation of the true possibility of
```math
    <p align="center">
    \frac{P(X_n, X_{n-1},..., X_{n-k})}{P(X_{n-1}, X_{n-2},..., X_{n-k})}
    </p>
``` 
is better becauses we got more samples.
...
For Autoregressive language model, we want to maximize the probability of the sentences in corpus under other parametrized model $P_\theta(X_{n}|X\_{n-1}, X\_{n-2},..., X\_{1})$. So we want to maximize
```math
    <p align="center">
    P(X_{n}, X_{n-1}, \ldots , X_{1})
    </p>
``` 
which can be factorized into
```math
    <p align="center">
    P(X_{n}|X_{n-1}, X_{n-2}, \ldots, X_{1})P(X_{n-1}|X_{n-2}, \ldots, X_{1}) \cdots P(X_{1})
    </p>
``` 
Maximizing this can be writen into minimizing the negative log-likelihood which is the loss function of RNN.
